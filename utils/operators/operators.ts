...
export const num = (val: number | string): ts.NumericLiteral =>
  ts.createNumericLiteral(val.toString());

export const str = (val: number | string): ts.StringLiteral =>
  ts.createStringLiteral(val.toString());
...
export const _if = (
  ...
  ts.createIf(
    expression,
    Array.isArray(thenStatement)
      ? ts.createBlock(thenStatement, true)
      : ts.createBlock([thenStatement], true),
    elseStatement
